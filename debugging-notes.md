...
Both bugs were invisible from the workflow's execution status, as n8n reported success both times. They only showed up by actually reading the output against what I knew the underlying accounts looked like. That's the same instinct a CSM needs when a health score dashboard says everything's green but a specific account clearly isn't: trust the specifics over the summary.
